...
<nav class="navbar navbar-inverse navbar-fixed-top">
  <div class="container-fluid">
    ...
    <ul class="nav navbar-nav">
      <li class="active"><a href="welcome.php"><?php if(!isset($_SESSION["username"])) {echo "dirasa";} else {echo "Home";}?></a></li>
...
<!-- The login button and signup buttons -->
<?php
  ...
  if (isset($_SESSION["username"]))
  {
	  //setting up credits updation from database;
	...
	//TESTING PURPOSE : echo "<h1 style=\"margin-top:100px;\"> <a href=\"redirect.php\">LOGOUT</a></h1>";
  }
?>
...
<div class="form-popup" id="myForm">
  <form method="POST" action="<?php echo $login_script ?>" class="form-container">
	...
	<input type="hidden" id="redirect" name="redirect" value="welcome.php" />
    ...
    <div class="chodal-footer"> 
	  <form action="./redirect.php" method="post" id="logout">
	   <div id="chodal_logout">
	    <input type="hidden" name="redirect_link" id="redirect_link" value="./welcome.php">
...
<!-- BELOW CODE WILL DISPLAY IF GUY IS LOGGED IN -->
<div id="content1" style="<?php if(!isset($_SESSION['username'])) {echo $dispnone;} ?>">
<h1> YOUR COURSES</h1>
<?php 
   include "authenticate_header.php";
   $uname = $_SESSION['username'];
   $query = "SELECT url, course_student.course_id, professor_unamer, name FROM course_student, course where course_student.course_id = course.course_id and course_student.student_uname = '$uname'";
   $result = mysqli_query($conn,$query) or die("Error in query or accessing elements.");
   if(mysqli_num_rows($result)>0)
   {
	   while($row = mysqli_fetch_assoc($result))
	   {
		 
		  echo "<a href='./courses/".$row['url'].".php'>■".$row['name']."<br/></a>";
	   }
   }
...
<!-- BELOW CODE WILL DISPLAY IF GUY IS NOT LOGGED IN -->
<div id="content2" style="<?php if(isset($_SESSION['username'])) {echo $dispnone;} ?>">
<h1> Welcome to dirasa... </h1>
<p>
	...A website for the students, of the students and by the student. Once
you sign up with us you can access the contents and even buy courses with the 10000 credits you receive on signup.
<br> You will learn there are many ways to earn credits on this website!	
</p>
</div>

<!-- ENDS HERE MAINPAGE CONTENT -->
</body>
